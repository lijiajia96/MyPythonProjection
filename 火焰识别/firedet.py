...
def find_countours_center1(frame,countours):
    C=[]#放中心点
    x_list = []
    y_list = []
    for j in range(np.array(countours).shape[0]):#将一个闭合曲线里面的所有x,y轴坐标放在两个list里
        x_list.append(countours[j][0][0])
        y_list.append(countours[j][0][1])
...
###############画框圈火焰##############
contours, hierarchy = cv.findContours(gray_fireImg, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE); #CV_RETR_EXTERNAL,CV_CHAIN_APPROX_NONE
len_contours=np.array(contours).shape;

for i in range(0,len(contours)):
    s=cv.contourArea(contours[i])
    c=cv.arcLength(contours[i], True)
    if c!=0:
        d=(4*3.1415926*s)/(c*c)
        if d<0.8:
            find_countours_center1(frame, contours[i])
            cv.drawContours(frame, contours[i], -1, (0, 255, 0), 3);
